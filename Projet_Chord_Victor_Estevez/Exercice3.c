...
void simulateur(int nb_proc) { //Initialisation du systeme
    srand(time(NULL));
    int inUse[nb_proc-1];
    int inUseMemorySponge[nb_proc];
    int max=1;
    for(int i=0;i<M;i++){   // max = (2^M)
        max=max*2;
    }
    printf("nombres de valeurs possibles: 2^M=%d, tirage de %d id uniques...\n",max, nb_proc-1);
    ...
    //Cette fois on ne tri pas l'id chord du processus qui cherchera a s'inserer, il restera à la fin du tableau.
    ...
    //le pair a l'indice size-1 ne sera pas prit en compte dans les calculs de Finger Table suivants.
    ...
    printf("inversep de %d [de taille %d]:___\n",i,pos);
        for(int i=0;i<pos;i++){
            printf("| %d ",inverse[i]);
        }
        printf("|\n");
...
void insertion(int nb_proc, int rang) {
    ...
    if(rang<nb_proc-1){
        ...
        MPI_Recv(&p, 1, MPI_INT, 0, TAGINIT, MPI_COMM_WORLD, &status); //reception de la taille de inversep
        MPI_Recv(&inversep, p, MPI_INT, 0, TAGINIT, MPI_COMM_WORLD, &status); //et de la table inversep
